...
var typescriptSupported = false;
var typescriptModule = null;
try {
...
try {
  ...
  if (typescriptModule && typescriptModule.register) typescriptModule.register();
  typescriptSupported = true;
} catch (e) {}

if (coffeeSupported || typescriptSupported) {
  filesRegEx = new RegExp('.(js' + (coffeeSupported ? '|coffee' : '') + (typescriptSupported ? '|ts' : '') + ')');
}
...
Migration.loadFromFilesystem = function(dir, internals, callback) {
  ...
  fs.readdir(dir, function(err, files) {
    ...
    var coffeeWarn = true;
    var typescriptWarn = true;
    files = files.filter(function(file) {
      if (coffeeWarn && !coffeeSupported && /\.coffee$/.test(file)) {
        log.warn('CoffeeScript not installed');
        coffeeWarn = false;
      }
      if (typescriptWarn && !typescriptSupported && /\.ts$/.test(file)) {
        log.warn('TypeScript not installed');
        typescriptWarn = false;
      }
