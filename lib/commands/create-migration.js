...
async function createMigrationDir (dir) {
  const res = await stat(dir).catch(() => {
    return { err: true };
  });
  if (res && res.err === true) {
    await mkdirp(dir);
...
async function executeCreateMigration (internals, config) {
  ...
  if (internals.migrationMode && internals.migrationMode !== 'all') {
    migrationsDir = `${internals.argv['migrations-dir']}/${internals.migrationMode}`;
  }

  if (internals.argv._.length === 0) {
    const error = `'migrationName' is required.`;
    log.error(error);
  ...
  internals.argv.title = folder[folder.length - 1];
  migrationPath = migrationsDir;

  if (folder.length > 1) {
    migrationPath += '/';

    for (let i = 0; i < folder.length - 1; ++i) {
      migrationPath += folder[i] + '/';
    }
  }
  internals.argv['migrations-dir'] = migrationPath;

  try {
    await createMigrationDir(migrationPath);
  } catch (err) {
    const error = `Failed to create migration directory at ${migrationsDir} => ${err}`;
    throw new Error(error);
  }
  ...
  if (!pluginTemplate && !customWrite) {
    const migration = new Migration(
      `${internals.argv.title}${(shouldCreateCoffeeFile(internals, config) ? '.coffee' : '.js')}`,
      migrationPath,
    ...
    log.info(util.format(`Created migration at ${migration.file.path}`));
...
async function createSqlFiles (internals, config) {
  ...
  const sqlDir = path.join(migrationsDir, 'sqls');
  try {
