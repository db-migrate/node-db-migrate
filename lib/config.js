...
exports.loadFile = function (fileName, currentEnv, plugins) {
  ...
  try {
    ...
    if (plugins) { plugin = plugins.overwrite('init:config:overwrite:require'); }
...
exports.loadObject = function (_config, currentEnv) {
  ...
  for (var env in config) {
    if (config[env].ENV) {
      if (!process.env[config[env].ENV]) { log.verbose('Environment variable ' + config[env].ENV + ' is empty!'); } else { out[env] = parseDatabaseUrl(process.env[config[env].ENV]); }
