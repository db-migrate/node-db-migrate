...
const setCurrent = (exports.setCurrent = function (env) {
...
function walkConfig (level) {
  for (const configEntry in level) {
...
exports.loadObject = function (_config, currentEnv) {
  ...
  for (const env in config) {
    ...
    if (config[env].overwrite || config[env].addIfNotExists) {
      const overwrite = config[env].overwrite || {};

      if (config[env].addIfNotExists) {
        const addIfNotExists = config[env].addIfNotExists;
