    ...
    if (typeof(config[env]) == "string") {
      ...
      for (var configEntry in config[env]) {
        if (config[env][configEntry].ENV != null){
          if (config[env][configEntry].ENV == "DATABASE_URL") {
            config[env][configEntry] = process.env[config[env][configEntry].ENV];
            // Parse the DB URL and put the components into the config as individual properties
            var dbParams = parseDatabaseUrl(config[env][configEntry]);
            for (var attr in dbParams) { config[env][attr] = dbParams[attr]; }
          }
          else 
            config[env][configEntry] =  process.env[config[env][configEntry].ENV];
        }
