...
var MysqlDriver = Base.extend({
  ...
  addMigrationRecord: function (name, migrationTable, callback) {
    var formattedDate = moment(new Date()).format('YYYY-MM-DD HH:mm:ss');
    this.runSql('INSERT INTO `' + migrationTable + '` (`name`, `run_on`) VALUES (?, ?)', [name, formattedDate], callback);
  },
   ...
   * @param callback
   */
  deleteMigration: function(migrationName, migrationTable, callback) {
