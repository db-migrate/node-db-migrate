...
var PgDriver = Base.extend({
      ...
      this.runSql('select version() as version', function(err, result) {
        ...
        this.runSql("SHOW search_path", function(err, result) {
            ...
            this.runSql('SET search_path TO ' + searchPath, function() {
                this.runSql("SELECT table_name FROM information_schema.tables WHERE table_name = '" + global.migrationTable + "'", function(err, result) {
                  if (err) {
                    return callback(err);
                  }
    ...
    all: function() {
		 ...
		 *
		 * @param migrationTable	- The name of the migration table
		 * @param callback
		 */
		allLoadedMigrations: function(migrationTable, callback) {
  		var sql = 'SELECT * FROM ' + migrationTable + ' ORDER BY run_on DESC, name DESC';
			return this.connection.query.apply(this.connection, sql);
		},
		 ...
		 * @param migrationTable	- The name of the migration table
		 */
		deleteMigration: function(migrationName, migrationTable) {
			var sql = 'DELETE FROM ' + migrationTable + ' WHERE name = ?';
			this.runSql(sql, [migrationName]);
		},
