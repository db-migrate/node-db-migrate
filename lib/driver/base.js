...
module.exports = Base = Class.extend({
  ...
  allLoadedSeeds: function(callback) {
    var sql = 'SELECT * FROM "' + internals.seedTable + '" ORDER BY run_on DESC, name DESC';
    return this.all(sql, callback);
  },
    ...
    *               column: [
                    {
                      ...
                      searchValue: "12",
                      searchValue: { table: "source" },
                      link: "AND", //defaults to AND if unset
                      selector: "id" //defaults to id if from searchvalue is a table
                    }
                    ]
                  }
    */
  remove: function(table, ids, callback) {
    ...
    if (util.isArray(ids) && typeof(ids[0]) !== 'object') {
        ...
        for (var column in ids) {
            ...
            column.selector = column.selector || 'id';
            ...
            searchClause += ' ' + preLink + ' "' + column.name + '" ' + column.operator;

            if(typeof(searchValue === 'object' && typeof(searchValue.table) === 'string') {

              searchClause += ' (' + column.searchValue.table + ')';
            }
            else {

              searchClause += ' (' + column.searchValue + ')'; 
            }
    ...
    return this.runSql(sql + searchClause).nodeify(callback);
  },
  ...
  deleteSeed: function(seedName, callback) {
    var sql = 'DELETE FROM "' + internals.seedTable + '" WHERE name = ?';
