...
module.exports = Base = Class.extend({
  ...
  createOtherOptions: function(options) {
    // This may be implemented inside drivers for database-specific features
    return '';
  },
  ...
  createTable: function(tableName, options, callback) {
    ...
    var otherOptions = this.createOtherOptions(tableOptions);
    ...
    var sql = util.format(
      'CREATE TABLE %s %s (%s%s) %s', ifNotExistsSql, tableName, columnDefs.join(', '), pkSql, otherOptions);
