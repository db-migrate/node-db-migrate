...
var MongodbDriver = Base.extend({
  ...
  createCollection: function(collectionName, callback) {
		console.log('in createCollection for = '+collectionName);
    log.verbose('creating collection:', collectionName);
    /*
    ...
    //this.runSql(sql, callback);
		this.run('createCollection', collectionName, null, callback);
  },
...
/*
      ...
      this.runSql(sql, callback);*/
		...
		this.run('renameCollection', collectionName, newCollectionName, callback);
  },
  ...
  addColumn: function(tableName, columnName, columnSpec, callback) {
	...
	dropCollection: function(collectionName, callback) {		
		this.run('dropCollection', 'event', null, callback);
	},
  ...
  runSql: function() {
	...
	run: function(command, collection, newCollection, callback) {
		
		console.log('in run');
		console.log('command = '+command);
		console.log('collection = '+collection);
		
		//var queryParams = collection;
		
		/*
		if(newCollection) {
					queryParams = collection + ', ' + newCollection;
				}*/
    ...
    if(global.dryRun) {
      return callback();
		...
		this.connection.connect(connectionString, function(err, db) {
			...
			//console.log('db[command] = '+db[command]);
			//console.log('collection = '+collection);
			...
			if(newCollection) {
				...
				db[command](collection, newCollection, function(err, data) {
