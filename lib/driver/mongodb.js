...
var MongodbDriver = Base.extend({
  ...
  _run: function(command, collection, options, callback) {
    ...
    var serverOptions = this.connection.serverOptions;

    // Get a connection to mongo
    this.connection.connect(connectionString, { "server": serverOptions }, function(err, db) {
  ...
  deleteMigration: function(migrationName, callback) {
    this._run('remove', global.migrationTable, {name: migrationName}, callback);
  },
  ...
  close: function(callback) {
    ...
    console.log('Not closing connection for mongodb driver v2.');
    callback();
  }
});
...
exports.connect = function(config, callback) {
  ...
  if(config.replSet === undefined) {
    mongo_set = 'localhost';
  } else {
    mongo_set = config.replSet;
  }
  ...
  mongoString += mongo_set + '/' + config.database;

  if (config.authenticationDatabase !== undefined) {
    mongoString += '?authSource=' + config.authenticationDatabase;
  }

  var options = {
    auto_reconnect: true,
    socketOptions: {
      connectTimeoutMS: config.MONGO_CONNECT_TIMEOUT || 5 * 60 * 1000,
      socketTimeoutMS: config.MONGO_SOCKET_TIMEOUT || 15 * 60 * 1000
    }
  };

  if(config.replSet !== undefined && config.sslCA !== undefined) {
    mongoString += '&ssl=true';
    options.ssl = true;
    options.sslValidate = true;
    options.sslCA = [fs.readFileSync(config.sslCA)];
    options.replSet = {
      poolSize: config.poolSize || 30
    }
  }

  var hosts = config.replSet !== undefined ? config.replSet.split(',') : [host];
  ...
  var mongoClient;
  if(servers.length > 2) {
    mongoString += '&readPreference=secondaryPreferred';
    mongoClient = new MongoClient(new ReplSet(servers, options));
  } else {
    mongoClient = new MongoClient(servers[0]);
  }

  mongoClient.serverOptions = options;
  console.log('mongoString', mongoString);
  callback(null, new MongodbDriver(mongoClient, mongoString));
};
