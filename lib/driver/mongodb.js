...
exports.connect = function(config, callback) {
  ...
  if(config.sslCA === undefined) {
    throw new Error('must supply path to CA certificate file for ssl connections');
  } else {
  ...
  var options = {
    ...
    socketOptions: {
      connectTimeoutMS: config.MONGO_CONNECT_TIMEOUT || 2 * 60 * 1000,
      socketTimeoutMS: config.MONGO_SOCKET_TIMEOUT || 15 * 60 * 1000
    },
    ssl: true,
    sslValidate: true,
    sslCA: [fs.readFileSync(config.sslCA)]
  };

  var servers = [];
  if(config.replSet !== undefined) {
    var serverNames = config.replSet.split(',');

    for(var i = 0; i < serverNames.length; ++i) {
      servers.push(new Server(serverNames[i], port, options));
    }
  } else {
