...
Migrator.prototype = {
  ...
  up: function(funcOrOpts, callback, printToRunOnly) {
    if (dbmUtil.isFunction(funcOrOpts)) {
      funcOrOpts(this.driver, callback);
    } else {
      this.upToBy(funcOrOpts.destination, funcOrOpts.count, callback, printToRunOnly || false);
    }
  },
  ...
  upToBy: function(partialName, count, callback, printToRunOnly) {
    var self = this;
    Migration.loadFromFilesystem(self.migrationsDir, function(err, allMigrations) {
      ...
      Migration.loadFromDatabase(self.migrationsDir, self.driver, function(err, completedMigrations) {
        ...
        if (printToRunOnly) {
          toRun.forEach(function(mig) {
            console.log('migration:' + mig.name);
          });
          callback(null);
          return;
        }
