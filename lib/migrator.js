...
Migrator.prototype = {
  ...
  up: function(funcOrOpts, printToRunOnly, callback) {
    if (dbmUtil.isFunction(funcOrOpts)) {
      funcOrOpts(this.driver, callback);
    } else {
      this.upToBy(funcOrOpts.destination, funcOrOpts.count, printToRunOnly, callback);
    }
  },
  ...
  upToBy: function(partialName, count, printToRunOnly, callback) {
    var self = this;
