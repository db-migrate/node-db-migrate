...
Migrator.prototype = {
  ...
  mark: function(funcOrOpts, callback) {
    if (dbmUtil.isFunction(funcOrOpts)) {
      funcOrOpts(this.driver, callback);
    } else {
      this.markToBy(funcOrOpts.destination, funcOrOpts.count, callback);
    }
  },
  ...
  markBy: function(count, callback) {
    this.markToBy('99999999999999', count, callback);
  },

  markTo: function(partialName, callback) {
    this.markToBy(partialName, Number.MAX_VALUE, callback);
  },

  markToBy: function(partialName, count, callback) {
    var self = this;
    Migration.loadFromFilesystem(self.migrationsDir, function(err, allMigrations) {
      ...
      Migration.loadFromDatabase(self.migrationsDir, self.driver, function(err, completedMigrations) {
        ...
        async.forEachSeries(toRun, function(migration, next) {
          log.verbose('preparing to mark up migration:', migration.name);
